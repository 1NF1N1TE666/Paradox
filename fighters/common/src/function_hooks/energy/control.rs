...
unsafe fn control_update(energy: &mut FighterKineticEnergyControl, boma: &mut BattleObjectModuleAccessor) {
    ...
    if !*(status_module as *const bool).add(0x12a) {
        if StatusModule::situation_kind(boma) == *SITUATION_KIND_AIR {
            ...
            && StatusModule::status_kind(boma) <= 0x1DB {
...
unsafe fn control_initialize(energy: &mut FighterKineticEnergyControl, boma: &mut BattleObjectModuleAccessor) {
    ...
    match reset_type {
        FallAdjust | FallAdjustNoCap | StopCeil | WallJump => {
            ...
            let run_speed_max = WorkModule::get_param_float(boma, hash40("run_speed_max"), 0);
            let ratio = VarModule::get_float(boma.object(), vars::common::instance::JUMP_SPEED_RATIO);
            let mut jump_speed_x = if WorkModule::get_param_float(boma, smash::hash40("air_speed_x_stable"), 0) > WorkModule::get_param_float(boma, hash40("jump_speed_x_max"), 0) {WorkModule::get_param_float(boma, smash::hash40("air_speed_x_stable"), 0)} else {WorkModule::get_param_float(boma, hash40("jump_speed_x_max"), 0)};
            let mut jump_speed_x_max = run_speed_max * ratio;
            let air_x_speed_max = if !WorkModule::is_flag(boma, *FIGHTER_INSTANCE_WORK_ID_FLAG_JUMP_NO_LIMIT) && energy.unk[2] == 0 {WorkModule::get_param_float(boma, smash::hash40("common"), smash::hash40("air_speed_x_limit"))} else {if boma.is_status(*FIGHTER_STATUS_KIND_JUMP) {jump_speed_x_max} else {-1.0}};
