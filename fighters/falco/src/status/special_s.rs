...
pub unsafe extern "C" fn special_s_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.off_flag(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
    fighter.set_int(-1, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
    fighter.set_int(*FIGHTER_FALCO_ILLUSION_STEP_START, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
    fighter.set_int(-1, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    ...
    if !StopModule::is_stop(fighter.module_accessor) {
        fighter.dec_int(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
    }
...
pub unsafe extern "C" fn special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let step = fighter.get_int(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        ...
        if fighter.global_table[PREV_SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
            if step == *FIGHTER_FALCO_ILLUSION_STEP_FORCE_END {
                fighter.set_int(*FIGHTER_FALCO_ILLUSION_STEP_END, *FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
                cont = true;
            ...
            } else if step == *FIGHTER_FALCO_ILLUSION_STEP_RUSH || step == *FIGHTER_FALCO_ILLUSION_STEP_START {
    ...
    if cont {
        if !StatusModule::is_changing(fighter.module_accessor) {
            if !fighter.is_flag(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_RUSH_FORCE_END) {
                if MotionModule::is_end(fighter.module_accessor) {
                    fighter.inc_int(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_INT_STEP);
                    fighter.off_flag(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
                }
            }
        }
...
pub unsafe extern "C" fn special_s_change_mot(fighter: &mut L2CFighterCommon, mot: Hash40) {
    if !fighter.is_flag(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE) {
        MotionModule::change_motion(fighter.module_accessor, mot, 0.0, 1.0, false, 0.0, false, false);
        fighter.on_flag(*FIGHTER_FALCO_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
    } else {
