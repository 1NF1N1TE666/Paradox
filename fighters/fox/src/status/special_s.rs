...
pub unsafe extern "C" fn special_s_main(fighter: &mut L2CFighterCommon) -> L2CValue {
    fighter.off_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
    fighter.set_int(-1, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
    fighter.set_int(*FIGHTER_FOX_ILLUSION_STEP_START, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
    fighter.set_int(-1, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    ...
    fighter.set_float(illusion_rush_speed_mul * illusion_rush_speed_mul_power_up, *FIGHTER_STATUS_WORK_ID_FLOAT_RESERVE_KINETIC_MOTION_SPEED_MUL);
    if !StopModule::is_stop(fighter.module_accessor) {
        fighter.dec_int(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
    }
...
pub unsafe extern "C" fn special_s_main_loop(fighter: &mut L2CFighterCommon) -> L2CValue {
    ...
    let step = fighter.get_int(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
    ...
    if !StatusModule::is_changing(fighter.module_accessor) {
        ...
        if fighter.global_table[PREV_SITUATION_KIND].get_i32() != *SITUATION_KIND_GROUND {
            if step == *FIGHTER_FOX_ILLUSION_STEP_FORCE_END {
                fighter.set_int(*FIGHTER_FOX_ILLUSION_STEP_END, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
                cont = true;
            } else if step == *FIGHTER_FOX_ILLUSION_STEP_END {
                if is_end && situation == *SITUATION_KIND_AIR {
                    fighter.change_status(FIGHTER_STATUS_KIND_FALL.into(), true.into());
                    fighter.set_float(3.0, *FIGHTER_INSTANCE_WORK_ID_FLOAT_LANDING_FRAME);
                    return 0.into();
                }
    ...
    if cont {
        if !StatusModule::is_changing(fighter.module_accessor) {
            if !fighter.is_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_RUSH_FORCE_END) {
                if MotionModule::is_end(fighter.module_accessor) {
                    fighter.inc_int(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
                    fighter.off_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
                }
            }
        }
        ...
        if situation != *SITUATION_KIND_GROUND {
            ...
            if step == *FIGHTER_FOX_ILLUSION_STEP_END {
                ...
                GroundModule::correct(fighter.module_accessor, GroundCorrectKind(*GROUND_CORRECT_KIND_GROUND));
                let motion_mul = fighter.get_float(*FIGHTER_STATUS_WORK_ID_FLOAT_RESERVE_KINETIC_MOTION_SPEED_MUL);
...
pub unsafe extern "C" fn special_s_change_mot(fighter: &mut L2CFighterCommon, mot: Hash40) {
    if !fighter.is_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE) {
        MotionModule::change_motion(fighter.module_accessor, mot, 0.0, 1.0, false, 0.0, false, false);
        fighter.on_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
    } else {
...
pub unsafe extern "C" fn special_s_exec(fighter: &mut L2CFighterCommon) -> L2CValue {
	let situation = fighter.global_table[SITUATION_KIND].get_i32();
    let set = if situation != *SITUATION_KIND_AIR {WorkModule::off_flag} else {WorkModule::on_flag};
    set(fighter.module_accessor, *FIGHTER_FOX_INSTANCE_WORK_ID_FLAG_ILLUSION_LANDING);
    let step = fighter.get_int(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
    let prev_step = fighter.get_int(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    if step != prev_step {
        special_s_handle_step(fighter);
    }
    if step == *FIGHTER_FOX_ILLUSION_STEP_START {
        if situation == *SITUATION_KIND_AIR {
            let stop_y_frame = fighter.get_int(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
            if stop_y_frame == 0 {
        ...
        fighter.on_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_CONTINUE);
        special_s_handle_step(fighter);
        fighter.set_int(*FIGHTER_FOX_ILLUSION_STEP_FORCE_END, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
    } else if step == *FIGHTER_FOX_ILLUSION_STEP_RUSH {
        if fighter.global_table[PAD_FLAG].get_i32() & *FIGHTER_PAD_FLAG_SPECIAL_TRIGGER != 0 {
            fighter.on_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_RUSH_FORCE_END);
        }
        if !fighter.is_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_RUSH_FORCE_END) {
            return 0.into();
        }
...
pub unsafe extern "C" fn special_s_handle_step(fighter: &mut L2CFighterCommon) {
    let step = fighter.get_int(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP);
    let situation = fighter.global_table[SITUATION_KIND].get_i32();
    fighter.set_int(step, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STEP_PREV);
    if step == *FIGHTER_FOX_ILLUSION_STEP_START {
        let illusion_stop_y_frame = WorkModule::get_param_int(fighter.module_accessor, hash40("param_special_s"), hash40("illusion_stop_y_frame"));
        fighter.set_int(illusion_stop_y_frame, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
        ...
        if situation != *SITUATION_KIND_AIR {
            sv_kinetic_energy!(set_speed, fighter, FIGHTER_KINETIC_ENERGY_ID_STOP, speed_x * illusion_start_x_mul, 0.0);
        } else {
            sv_kinetic_energy!(reset_energy, fighter, FIGHTER_KINETIC_ENERGY_ID_STOP, 6, speed_x * illusion_start_x_mul, 0.0, 0.0, 0.0, 0.0);
            ...
            if illusion_stop_y_frame != 0 {sv_kinetic_energy!(set_speed, fighter, FIGHTER_KINETIC_ENERGY_ID_GRAVITY, 0.0);
        ...
        let illusion_end_air_stop_y_frame = WorkModule::get_param_int(fighter.module_accessor, hash40("param_special_s"), hash40("illusion_end_air_stop_y_frame"));
        fighter.set_int(illusion_end_air_stop_y_frame, *FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_INT_STOP_Y_FRAME);
        if situation != *SITUATION_KIND_AIR {
...
pub unsafe extern "C" fn special_s_air_control(fighter: &mut L2CFighterCommon) {
    if fighter.is_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_AIR_CONTROL) {
        ...
        fighter.off_flag(*FIGHTER_FOX_ILLUSION_STATUS_WORK_ID_FLAG_AIR_CONTROL);
    }
}
